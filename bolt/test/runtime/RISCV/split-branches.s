...
# RUN:   --reorder-functions=cdsort --split-function --split-all-cold \
...
# RUN: llvm-bolt %t.exe -o %t.all.bolt --split-function --split-strategy=all
# RUN: %t.all.bolt
# RUN: llvm-bolt %t.exe -o %t.random.bolt --split-function \
# RUN:   --split-strategy=randomN --bolt-seed=1
# RUN: %t.random.bolt
# RUN: llvm-mc -triple=riscv32 -filetype=obj %s -o %t.rv32.o
# RUN: llvm-strip --strip-unneeded %t.rv32.o
# RUN: ld.lld --emit-relocs %t.rv32.o -o %t.rv32.exe
# RUN: llvm-bolt %t.rv32.exe -o %t.rv32.bolt --data=%t.fdata --reorder-blocks=ext-tsp \
# RUN:   --reorder-functions=cdsort --split-functions --split-all-cold \
# RUN:   --pad-funcs-before=foo:2097152 | FileCheck %s
# RUN: llvm-objdump -d --disassemble-symbols=foo,foo.cold.0 %t.rv32.bolt | FileCheck %s --check-prefix=CODE
